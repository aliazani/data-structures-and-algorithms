...
@DisplayName("PriorityQueueWithArray")
class PriorityQueueWithArrayTest {
    PriorityQueueWithArray<Integer> queue;

    @BeforeEach
    void setUp() {
        queue = new PriorityQueueWithArray<>(Integer.class, 3);
    }
    ...
    void enqueue_queueIsNotFull_addElementToItsCorrectPosition() {
        queue.enqueue(1);
        assertEquals("[1, null, null]", queue.toString());
    ...
    void toString_queueIsEmpty_returnEmptyQueueString() {
        assertEquals("[null, null, null]", queue.toString());
    }
    ...
    void toString_queueHasWrappedAround_returnQueueStringWithWrappedElements() {
        ...
        assertEquals("[10, 20, 40]", queue.toString());
    }
}